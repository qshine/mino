...
// Run renders input and streamed answers around a synchronous message handler.
	...
	for {
		...
		case line, ok := <-lines:
			...
			err := respond(ctx, prompt, func(delta string) error {
				_, writeErr = fmt.Fprint(output, Text(delta))
				return writeErr
			})
			if writeErr != nil {
				return errors.New("Failed to write terminal output")
			...
			if err != nil {
				fmt.Fprintln(errorOutput, "Error: "+Text(err.Error()))
				continue
			}
		}
	}
