...
if [[ $# -gt 1 ]]; then
  fail 'Usage: bash install.sh [chapter-NN[.PATCH] | 0.N.PATCH]'
fi
if [[ ${1:-} == --help || ${1:-} == -h ]]; then
  printf 'Usage: bash install.sh [chapter-NN[.PATCH] | 0.N.PATCH]\nInstalls the latest release by default.\n'
  exit 0
fi
...
# Chapter tags name the source; the executable and archives keep numeric versions.
chapter_pattern='^chapter-(0[1-9]|[1-9][0-9])(\.([1-9][0-9]*))?$'
version_pattern='^v?0\.([1-9][0-9]?)\.(0|[1-9][0-9]*)$'
if [[ $release_tag =~ $chapter_pattern ]]; then
  chapter=$((10#${BASH_REMATCH[1]}))
  patch=${BASH_REMATCH[3]:-0}
elif [[ $release_tag =~ $version_pattern ]]; then
  chapter=${BASH_REMATCH[1]}
  patch=${BASH_REMATCH[2]}
else
  fail 'Invalid release. Use chapter-01, chapter-01.1, or 0.1.0.'
fi
release_version="0.$chapter.$patch"
printf -v release_tag 'chapter-%02d' "$chapter"
if [[ $patch != 0 ]]; then release_tag+=".$patch"; fi
