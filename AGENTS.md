...
- Root: `cmd/mino/main.go` is the entry point and `internal/app.go` composes the application. `internal/gateway/` owns terminal interaction and commands; `internal/agent/` owns model requests. Configuration and SOUL support remain in package `mino` under `internal/`. Root `assets.go` embeds `install.sh` and `SOUL.md`.
