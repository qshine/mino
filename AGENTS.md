...
- Conversation history: Chapter 02 uses `~/.mino/history.jsonl` and a stable `history.lock`, both `0600`. Every record has one consistent `session_id`; replay only completed turns. Preserve Responses output items for stateless replay. Backup incomplete tails before repair, reject middle corruption and mixed sessions, and stop chat on storage failures. Tests must isolate HOME and history files. Multi-session commands remain Chapter 04.
