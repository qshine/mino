...
- **Bilingual checks:** Match heading hierarchy and numbers, figure/table numbers, examples' meaning, version scope, and capability status. Translate example questions, instructions, and answers naturally; preserve fixed application prompts and API fields. Use corresponding-language relative links. Keep a small number of immutable source links as evidence, with labels describing the supported behavior; do not introduce symbols or sections just to link them. Check incoming anchors after heading changes.
