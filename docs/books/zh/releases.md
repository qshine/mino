...
根据所有者的明确要求，第一章重新发布为 `v0.1.0`，替换此前的第一章构建，包括 `v0.1.1` 和过渡版本。基线包含 OpenAI 官方 Go SDK、`cmd/mino` 启动入口、`internal/` 应用包、根目录的 `install.sh`，以及可编辑的 `~/.mino/SOUL.md` 身份。`assets.go` 嵌入安装脚本和默认身份，同时保留原 `v0.1.1` 中的私有下载修复。

如果使用过之前任一版本，即使 `mino version` 已显示 `0.1.0`，也要运行 `mino update v0.1.0`，因为版本号没有变化。如果旧版内嵌的更新器失败，请用当前 [README 中的安装命令](https://github.com/qshine/mino/blob/main/README.zh-CN.md#安装)重新安装。两种方式都会保留配置和自定义 `~/.mino/SOUL.md`。本次经所有者授权的重置完成后，后续修复使用新补丁标签，不改变已发布标签和附件。
...
用户可以通过 `mino update v0.1.0` 重装第一章基线。这只替换可执行文件，保留 `~/.mino/config.json` 和自定义 `~/.mino/SOUL.md`。

## 私有仓库安装

README 的安装命令通过已经登录的 `gh api` 从 `main` 读取根目录的 `install.sh`。安装器和内嵌的更新器使用 `gh` 获取 Release ID，再通过 GitHub 专门的附件接口下载文件，避免版本元数据中不完整的附件列表阻止安装。
