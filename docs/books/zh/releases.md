...
当前源码包含尚未发布的变更：从 `~/.mino/SOUL.md` 加载身份指令，并由 `assets.go` 嵌入根目录的安装脚本和默认身份。已发布的 `v0.1.0` 仍读取工作目录中的 `AGENTS.md`，更新到该标签不会启用 `SOUL.md`。替换可执行文件时，已有用户身份修改会保留。

## 第一章基线重置

根据所有者的明确要求，第一章重新发布为 `v0.1.0`，替换原来的 `v0.1.0`、`v0.1.1` Release 和标签。新基线引入 OpenAI 官方 Go SDK、`cmd/mino` 启动入口，以及 `internal/` 应用包，同时保留原 `v0.1.1` 中的私有下载修复。

9 月 24 日重新发布的版本包含应用直接位于 `internal/` 的最终目录布局，替换过渡版本 `v0.1.0`，运行行为没有变化。

如果使用过之前任一版本，可以运行 `mino update v0.1.0` 安装新基线；如果旧版内嵌的更新器失败，请用当前 [README 中的安装命令](https://github.com/qshine/mino/blob/main/README.zh-CN.md#安装)重新安装。两种方式都会保留已有配置。单看版本字符串无法区分这些 `0.1.0` 构建。这是一次性重置，后续修复仍使用新补丁标签，不改变已发布标签和附件。
