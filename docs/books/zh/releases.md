...
第 01–04 章于 **2026-09-26** 以 `chapter-01` 至 `chapter-04` 发布，提供 Apple Silicon 和 Intel 的 macOS 安装包。当前工作区的实现截止到第 03 章，各章链接到自己的发布源码。
...
完成这次安装后，`mino update` 选择最新发布版，`mino update chapter-03` 选择本章。安装器也接受 `0.3.0` 或 `v0.3.0`，两者都指向 `chapter-03`；这些是版本选择别名，不是额外的 Git 标签。
