...
加载配置和身份后，第二章开发版会打开 `~/.mino/history.jsonl`，恢复已完成的问答。所有记录属于同一个 `session_id`；已有记录时，启动会沿用这个 ID。首次启动的历史文件为空。Mino 还会打开固定的 `~/.mino/history.lock`，确保同一份历史同时只能由一个进程使用。如果启动时提示历史正在使用，请关闭另一个 Mino 进程；退出后锁文件可以继续存在，不应通过删除它绕过仍在运行的进程。
