...
先完成[准备与安装](../getting-started.md)。本章实验应使用开头注明的源码版本；`v0.2.0` 已经包含第二章的历史功能。接下来，你会看到自己提供了什么、模型收到什么，以及 Mino 怎样把控制权交还给你。
...
[第 02 章](./02-jsonl-history.md)在 `v0.2.0` 中解决历史缺失的问题：用 JSON Lines（JSONL）文件保存问答，启动时恢复已完成的问答，并随下一条问题一起发送。工具执行和 Agent 循环仍是第 03 章的规划内容，见[章节规划](../plan-todo-chapters.md)。
