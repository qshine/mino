...
适用范围：**第 02 章开发版实现** · 目标版本：**v0.2.0（Unreleased，尚未发布）** · 源码核对提交：**[c3d8e7b](https://github.com/qshine/mino/tree/c3d8e7b438483d1580fda0e39a093480b105b437)**
...
*会话*把属于同一段对话的问答组织在一起。*一轮问答*从一次提交的输入开始，成功时以完整回答结束。Mino 当前在 `~/.mino/history.jsonl` 中保存一个会话。文件本身就把其中的记录归为一段对话，重启时重新打开同一个文件。

JSON Lines（JSONL）每行保存一条 JSON 记录。Mino 按顺序追加记录，不会每问一次就重写整段对话。每条记录都带有 `turn_id`、从 1 开始连续递增的 `seq`，以及格式版本 `v: 1`。每轮问答会生成新的轮次 ID，由 32 个小写十六进制字符组成；同一轮内的所有记录共用这个 ID。记录已经归属于同一个文件，因此不需要再携带会话 ID。
...
| --- | --- | --- |
| `user_message` | 发送问题之前 | 在当前对话文件中开始新的一轮问答。 |
...
加载文件时，Mino 会检查序号、`turn_id` 和问答顺序。回答和结束记录必须匹配当前输入的轮次 ID，之后的新一轮问答则必须使用新的 ID。规划中的第四章会把各个会话保存在 `~/.mino/sessions/<session_id>.jsonl`，由文件名承载会话 ID；`/new` 将创建新的文件。
...
小屏幕上可以横向滚动图表。成功流程有[两次存储确认](https://github.com/qshine/mino/blob/c3d8e7b438483d1580fda0e39a093480b105b437/internal/conversation.go#L90)：Mino 在请求模型前同步输入，在接受下一个问题前同步回答和结束记录。写入或同步失败都会停止这条流程。
...
如果进程在保存结束记录前停止，下次启动时，Mino 会把未结束的一轮标记为 `interrupted`，排除在上下文之外。如果最后一行不完整或 JSON 语法损坏，程序先保存一份私有恢复副本，再移除这段尾部。文件中间的无效记录、未知字段或版本，以及无效的轮次 ID 或记录顺序，会使加载停止，历史内容保持不变。恢复过程从不自动重发模型请求。
...
[重启检查](https://github.com/qshine/mino/blob/c3d8e7b438483d1580fda0e39a093480b105b437/internal/conversation_test.go#L17)先提交颜色信息，关闭 Mino，再启动并追问颜色。测试会查看第二次请求，确认其中包含原始输入、返回的推理与回答项，以及新问题；还会检查保存的六条记录序号连续，且没有 `session_id` 字段。每轮内的记录共用有效的 `turn_id`，两轮之间则使用不同的 ID。**检查请求能证明对话连续性，单凭一个合理的回答不能。**
