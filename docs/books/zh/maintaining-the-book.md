...
- **交互优先**：源码用于核对行为，不作为章节提纲。省略仓库导览、程序入口讲解、文件或函数调用链、SDK 构造与辅助方法说明、依赖版本清单。解释读者能观察到的变化：提交问题后，程序发送当前输入和指令；收到的片段成为回答；完成或报错后，控制权回到读者。逐段检查它说明了什么输入、提供了什么上下文、如何处理响应、下一步由谁行动，或解释了哪个直接相关的工程选择。保留选择理由、代价、边界和验证依据，删除无关的实现说明。不要把读者不需要的代码导览搬到附录。
- **语气与密度**：称读者为“你”，用“Mino”“程序”“模型”明确动作主体。每段围绕一个意思，使用主动句和自然亲切的语气，句子长短有变化，不把每句话都拆成一段。先说明原因，再解释做法，首次出现的术语和缩写要定义。可以用一个真实疑问连接观察与解释。避免闲话、宣传语、刻意制造悬念、未经解释的行话和大段加粗。
...
- **双语检查**：标题层级与编号、图表编号、示例含义、适用版本和能力状态必须对应。两种语言保留相同的推理、取舍、证据边界和亲切感，各自自然表达，不机械翻译口头禅或复制断句。示例问题、指令和回答使用自然译文，固定程序提示和 API 字段保持不变。站内链接指向对应语言的相对路径。保留少量不可变的源码链接作为证据，链接文字说明它支持的行为，不为提供链接而引入符号或单设小节；标题变化后检查指向它的锚点链接。
...
写作 agent 借用 `khazix-writer` 的部分技巧：具体场景开头、逐步发现问题、有用的追问、长短句变化、自然衔接，以及结尾对开头交互的呼应。优先级是：**事实与证据 → 教学清晰度、工程推理和书籍规范 → 叙事表达**。起草或润色前，从当前技能目录或主 agent 提供的位置找到已安装的技能，阅读其说明和相关参考资料。若技能不可用，应说明情况并使用写作定义中的适配规则，不自行安装，也不声称已经加载。仓库不依赖个人机器上的安装路径。

适配后保留标题编号、有用的表格和列表、准确的代码与 API 字段，以及正常标点。不设篇幅、口头禅或独句成段的配额，也不要求脏话、自嘲、情绪化标点、哲学延伸或推广结尾。Mino 的作者仍是 **qqling | AI Builder**，不借用原技能作者的身份、经历、署名和联系方式。作者故事和当时的设计动机，只能来自用户提供或确认的材料。代码能证明当前行为，不能证明作者当时怎么想；agent 运行测试得到的是测试证据，不是作者的亲身经历。推断出的理由须明确作为分析，未确认的个人细节应省略；确实不可缺少时，请主 agent 先向用户确认再写。可以使用明确标注的交互示意。

每章在解释相关行为的位置，让主要工程取舍可见：当前问题、合理的可选方法、现在的选择及有依据的理由，以及代价或适用边界。把它连接到一项检查，说明检查能证明什么、不能证明什么。通常补进现有小节的几个短段落即可，小修复不必编造新取舍，也不另加一遍总结。例如，检查请求内容能看清程序提供了哪些历史，回答合理本身不能证明记忆处理正确；单个 JSONL 文件是当前对话范围的选择，不是格式的限制；存储失败后停止聊天，可以避免在历史无法可靠保存时继续运行，代价是中断聊天，但这不会撤回已显示的文字，也不保证部分记录已经落盘。使用这些例子前，须核对章节对应版本。

写作分三轮：先确认事实和证据边界，再组织交互、取舍与可观察的检查，最后润色叙事，不改变事实和必要结构。交稿前检查四项：事实与结构准确；读者能跟上并验证交互；选择的理由、代价和边界可见；表达自然简洁。小修复或辅助页面只检查与之相关的章节要求。将简短依据及技能加载或缺失情况报告给主 agent，不把风格报告附在章节末尾。这是 Mino 适配后的检查方式，不沿用原技能的评分表。
